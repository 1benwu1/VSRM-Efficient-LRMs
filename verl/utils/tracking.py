...
class _TensorboardAdapter:
    def __init__(self, proj_p, exp_p):
        import os
        ...
        tensorboard_dir = os.path.join(
            "/mnt/dolphinfs/hdd_pool/docker/user/hadoop-mtsearch-assistant/ai-search/yuechuhuai/verl_results",  # 根目录
            proj_p,
            exp_p,
            "logs",
            datetime.now().strftime("%Y%m%d"),  # 日期目录
            f"exp1_{datetime.now().strftime('%H%M%S')}"  # 带时间戳的实验名称
        )
