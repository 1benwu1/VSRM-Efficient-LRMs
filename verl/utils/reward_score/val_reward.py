...
def compute_math_val(data_source, solution_str, ground_truth, solution_len, extra_info=None) -> float:

    # score = 0
    try:
        import re
        think_pattern = re.compile(r'<think>(.*?)</think>', re.DOTALL)
        match = think_pattern.search(solution_str)


        category = ""


        result = strip_answer_string(extract_answer(solution_str))


        if match: 

            equal = math_equal(result,ground_truth)

            if equal:
                score= 1.0 
                category = "complete_think_ans_correct"

            else: 
                score= 0.0
                category = "complete_think_ans_wrong"


        else:
            incomplete_pattern = r'<think>(.*)'
            match2 = re.search(incomplete_pattern, solution_str, re.DOTALL)
            ...
            if match2: 
                ...
                if equal:
                    score= 1.0 
                    category = "incomplete_think_ans_correct"


                else: 
                    score= 0.0
                    category = "incomplete_think_ans_wrong"
                ...
                if equal:
                    score= 1.0 
                    category = "no_think_ans_correct"

                else: 
                    score= 0.0
                    category = "no_think_ans_wrong"
