...
from verl.workers.rollout_postprocess import roll_postprocess
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
            ...
            rollout_sharding_manager = FSDPVLLMShardingManager( # 这里 第二个参数
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        with self.rollout_sharding_manager:
            ...
            # if val!=1:
            #     post_p = roll_postprocess(self.tokenizer, self.config)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_sequences_train(self, prompts: DataProto):
        dprint("generate_sequences TRAIN!!")


        post_p = roll_postprocess(self.tokenizer, self.config)

        GTs = prompts.non_tensor_batch['reward_model']
        prompts = prompts.to(torch.cuda.current_device()) 
        # dprint("*************")
        ...
        with self.rollout_sharding_manager:
            ...
            output = post_p.postprocess_output(output, self.rollout , GTs)
